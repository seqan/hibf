...
This recipe creates a small program that
  ...
  2. "converts" the string to a range of library_template::dna5 (Bonus: throws an exception if loss of information occurs)
...
The easiest way to write to a sequence file is to use the library_template::sequence_file_output::push_back()
or library_template::sequence_file_output::emplace_back() member functions.
...
\cond DEV
The class library_template::sequence_file_output takes an extra parameter allowing to custom select the
...
- library_template::search_cfg::max_error_total: Maximum number of total errors
- library_template::search_cfg::max_error_substitution: Maximum number of substitutions
- library_template::search_cfg::max_error_insertion: Maximum number of insertions
- library_template::search_cfg::max_error_deletion: Maximum number of deletions
These are constructed with absolute numbers or rates:
- library_template::search_cfg::error_count: Absolute number of errors
- library_template::search_cfg::error_rate: Rate of errors \f$\in[0,1]\f$

To search for either 1 insertion or 1 deletion you can use the library_template::search_cfg::error_count:
...
3. For the resulting alignments, print which read was mapped against with reference id and the number of library_template::gap's
...
can occur since obviously some bases are not available. When converting to library_template::dna5 or library_template::rna5,
non-canonical bases (letters other than A, C, G, T, U) are converted to ``'N'`` to preserve ambiguity at that position.
For library_template::dna4 and library_template::rna4 there is no letter ``'N'`` to represent ambiguity, so the conversion from `char` for
IUPAC characters tries to choose the best fitting alternative (see library_template::dna4 for more details).
...
This will use `4` threads by default and can be adjusted by setting `library_template::contrib::bgzf_thread_count` to
the desired value:
...
Our alphabet library_template::dna4 cannot be easily auto-vectorized by the compiler.

See [this discussion](https://github.com/seqan/library_template/issues/1970) for more details.
