...
cmake3 "${WORK_DIR}/.." \
    -DHIBF_VERCEL_PREVIEW_DOC=ON \
    -DHIBF_DOXYGEN_EXTERNAL_SEARCH_URL="https://${VERCEL_URL}/api/doxysearch.sh" \
    -DCMAKE_INSTALL_PREFIX="" \
    -DCMAKE_INSTALL_DOCDIR="."  1>/dev/null

### Build documentation.
cmake3 --build . --target download-cppreference-doxygen-web-tag 1>/dev/null
ctest3 -j 4 --output-on-failure .

### Install documentation.
mkdir -p "${EXPORT_DIR}"
DESTDIR="${EXPORT_DIR}" cmake3 -DCOMPONENT=docs -P cmake_install.cmake 1>/dev/null
