...
[[nodiscard]] size_t hierarchical_binning::needed_technical_bins(size_t const requested_num_ub) const
{
    ...
    return tmax_without_empty_bins(needed, config.empty_bin_fraction);
}
...
size_t hierarchical_binning::add_lower_level(data_store & libf_data) const
{
    ...
    else
    {
        ...
        size_t const number_of_technical_bins = tmax_without_empty_bins(num_user_bins, config.empty_bin_fraction);
