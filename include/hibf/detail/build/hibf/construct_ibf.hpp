...
hibf::interleaved_bloom_filter<> construct_ibf(robin_hood::unordered_flat_set<size_t> & parent_kmers,
                                               ...
                                               bool is_root)
{
    auto bin_size_in_bits = [&](size_t const number_of_kmers_to_be_stored)
    {
        double const numerator{
            -static_cast<double>(number_of_kmers_to_be_stored * data.hibf_config.number_of_hash_functions)};
        double const denominator{std::log(1
                                          - std::exp(std::log(data.hibf_config.maximum_false_positive_rate)
